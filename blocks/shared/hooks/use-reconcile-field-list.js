/**
 * Keeps a selection of { key, ... } entries (gateway/data-cards' own
 * `facets` attribute) in sync with the columns currently available for a
 * post type or Collection: drops entries whose key no longer exists
 * (e.g. a meta field specific to a previously selected post type),
 * falling back to `defaultValue` (itself filtered the same way) if that
 ...
 * `isFilterable` fields (see that block's own `selectableFacetColumns`).
 */
