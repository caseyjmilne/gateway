 ...
 * routed: Root configured AND a real Template post exists for this model
 * (`PermalinkEditor.jsx`'s own copy already tells the site owner this
 * exact requirement) -- `Permalink_Routes::register_rules()` never
 * registers a rewrite rule for a model missing either one, so a link
 * built from Root alone would just 404. Unlike Root, "does a Template
 * exist" isn't something this function can answer from `fields` alone
 * anymore -- a Template post declares which Collection it's for via its
 * own post meta, not the other way around -- so callers fetch that
 * separately (`GET /gateway/v1/models/<class>/permalink`'s own
 * `available` flag, the same route `blocks/card-link/src/edit.js`
 * already uses for the identical question) and pass it in as `hasTemplate`.
 *
 ...
 * @param {object}  record      One of that model's own records, as returned
 *                               by the records list/detail endpoints.
 * @param {boolean} hasTemplate Whether this model currently has a real,
 *                               fully-routed Template (the caller's own
 *                               `GET .../permalink` fetch's `available`
 *                               flag).
 ...
 */
export function getRecordPermalink( fields, record, hasTemplate ) {
