...
class Column_Registry {
	...
	public static function get_columns_for_collection( $class_name ) {
		...
		foreach ( Model_Fields::all( $class_name ) as $field ) {
			...
			if ( $is_filterable ) {
				...
				// date) is the one meaningfully useful facet UI for it. A
	 ...
	 * it displays a `WP_Post` property in the broad sense -- is what lets a
	 * consumer render it as an `<img>` rather than plain text.
	 * `isFilterable => false` opts this column out of every Facets picker
	 * entirely -- filtering a grid by which rows happen to have a
	 * particular image doesn't mean anything.
	 *
	 * @param string $post_type Post type slug.
