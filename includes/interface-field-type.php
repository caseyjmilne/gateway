...
interface Field_Type {
	 ...
	 * `Datetime_Field_Type` all file there), but the six-category
	 * vocabulary stays fixed so a future type effectively picks its own
	 * home rather than the picker needing a seventh category invented
	 * for it.
	 *
	 * @return string
	 */
	 ...
	 * `Email_Field_Type`, `URL_Field_Type`, `Time_Field_Type`/
	 * `Datetime_Field_Type` (each a plain literal value typed/picked
	 * once, the SAME shape as these -- unlike `Date_Field_Type`'s own
	 * `'today'` sentinel below, no "current time"/"right now" option was
	 * ever asked for either of these two types), and -- per a direct
	 * request
